    ...
    <div id="menus">
        <el-menu @open="handleOpen" @close="handleClose" router :collapse="$store.state.menu.collapse" :default-active="index" v-if="index">
            <template v-for="menu in lists">
                <el-submenu :index="menu.id.toString()" v-if="menu.childs.length">
                    <template slot="title">
                        <i :class="'el-icon-' + menu.icon"></i>
                        <span v-text="menu.name"></span>
                    </template>
                    <el-menu-item :index="menuChild.id.toString()" :route="{path: menuChild.url}" v-for="menuChild in menu.childs" :key="menuChild.id">
                        <i :class="'el-icon-' + menuChild.icon"></i>
                        <span slot="title" v-text="menuChild.name"></span>
                    </el-menu-item>
                </el-submenu>
                <el-menu-item :index="menu.id.toString()" :route="{path: menu.url}">
                    <i :class="'el-icon-' + menu.icon"></i>
...
<script>
    export default {
        ...
        methods: {
            getLists() {
                this.$request.get(this.$url.home.menus).then(res => {
